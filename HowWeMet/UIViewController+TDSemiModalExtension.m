...
#define DEGREES_TO_RADIANS(x) (M_PI * (x)/180.0)
	...
	if(orientation == UIInterfaceOrientationLandscapeLeft ||
	   orientation == UIInterfaceOrientationLandscapeRight) {
        ...
        if(IS_WIDESCREEN)
            [modalView setBounds:CGRectMake(0, 0, 568, 300)];
        else
        ...
        if(IS_WIDESCREEN)
        {
            [modalView setBounds:CGRectMake(0, 0, 320, 504)];
            [coverView setFrame:CGRectMake(0, 0, 320, 504)];
        }
        else
        {
